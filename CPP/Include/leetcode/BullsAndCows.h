...
namespace leetcode
{
    ...
    {
        int counter_secret[10]{ 0 };
        int counter_guess[10]{ 0 };

        // it's easy to count the bulls,
        // using a single variable, each time find a matched chars in the same position, increase one.
        int bulls = 0;
        int cows = 0;

        for (int i = 0; i < secret.size(); i++)
        {
            if (secret[i] == guess[i])  // find a bull
            {
                bulls++;
            }
            else
        ...
        for (int i = 0; i < 10; i++)
        {
            cows += min(counter_secret[i], counter_guess[i]);
        }
